...
var _Sources = (() => {
  ...
  var ReadAllComics_exports = {};
  __export(ReadAllComics_exports, {
    ReadAllComics: () => ReadAllComics,
    ReadAllComicsInfo: () => ReadAllComicsInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/ReadAllComics/ReadAllComicsParser.ts
    ...
    parseMangaDetails($, mangaId) {
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          image,
          status: "Ongoing",
          // Default sicuro per questo sito
          author: "Unknown",
          artist: "Unknown",
          tags: tagSections,
          desc
    ...
    parseChapters($, mangaId) {
      ...
      for (const link of links) {
        ...
        chapters.push(App.createChapter({
          ...
          // Data non disponibile facilmente
          langCode: "en"
        }));
      }
      ...
      $(".type-post").each((_, el) => {
        ...
        if (id && title) {
          latestItems.push(App.createPartialSourceManga({
            ...
            image,
            title,
            subtitle: $el.find("h2 a").text().trim()
            // Mostra il titolo del capitolo come sottotitolo
          }));
        }
      });
  ...
  var ReadAllComicsInfo = {
    version: "1.1.0",
    // Bump version: Sorting & Parsing Fixes
    name: "ReadAllComics",
    ...
    // Il sito ha spesso ads o contenuti vari
  ...
  var ReadAllComics = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
      this.baseUrl = DOMAIN;
      this.parser = new ReadAllComicsParser();
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: chapterId,
        // ReadAllComics usa URL completi per i capitoli solitamente
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $ = this.cheerio.load(response.data);
  ...
  return __toCommonJS(ReadAllComics_exports);
})();
