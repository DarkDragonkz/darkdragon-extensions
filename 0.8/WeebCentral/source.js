...
var _Sources = (() => {
  ...
  var WeebCentralParser = class {
    ...
    parseChapters($) {
      ...
      $("#chapter-list > div").each((_, div) => {
        ...
        const time = dateStr ? new Date(dateStr) : /* @__PURE__ */ new Date();
    ...
    parseSearchResults($) {
      ...
      $("article").each((_, article) => {
        ...
        results.push(App.createPartialSourceManga({
          ...
          image,
          title,
          subtitle: subtitle ? this.decodeHTMLEntity(subtitle) : void 0
        }));
  ...
  var WeebCentral = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      const hasNextPage = manga.length >= limit ? { page: page + 1 } : void 0;
