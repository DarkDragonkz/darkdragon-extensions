...
var _Sources = (() => {
  ...
  // src/ReadComicsOnline/ReadComicsOnlineParser.ts
  ...
  var ReadComicsOnlineParser = class {
    parseMangaDetails($2, mangaId) {
      ...
      let desc = $2("div.manga.well p").text().trim();
      desc = desc.replace(/^Summary:\s*/i, "");
      const arrayTags = [];
      $2("a", "dd.tag-links").each((_, a) => {
    ...
    parseChapters($2, mangaId) {
      const chapters = [];
      $2("ul.chapters li").each((_, li) => {
        const title = $2("h5.chapter-title-rtl", li).text().trim();
        const link = $2("a", li).attr("href");
        let chapterId = link?.split("/").pop() ?? "";
        if (chapterId.includes("?")) chapterId = chapterId.split("?")[0];
        if (chapterId.includes("#")) chapterId = chapterId.split("#")[0];
        if (!chapterId) return;
        const dateText = $2("div.date-chapter-title-rtl", li).last().text().trim();
    ...
    parseHomeSections($2, sectionCallback) {
      ...
      const latestItems = [];
      $2("div.media", "div.list-container > div.row").each((_, item) => {
    ...
    sourceTags: [
      {
        ...
        type: import_types3.BadgeColor.BLUE
      }
    ],
  ...
  var ReadComicsOnline = class {
    constructor(cheerio) {
      ...
      this.requestManager = App.createRequestManager({
        ...
        requestTimeout: 2e4,
        // Aumentato a 20 secondi per evitare timeout
        interceptor: {
