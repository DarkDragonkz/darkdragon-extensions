...
var _Sources = (() => {
  ...
  var MangaBall_exports = {};
  __export(MangaBall_exports, {
    MangaBall: () => MangaBall,
    MangaBallInfo: () => MangaBallInfo
  });
  var import_types = __toESM(require_lib());

  // src/MangaBall/MangaBallParser.ts
  var DEFAULT_IMAGE = "https://paperback.moe/icons/logo-alt.svg";
  var MangaBallParser = class {
    parseCsrfToken(html) {
    ...
    parseMangaDetails($, mangaId) {
      ...
      const statusText = detailRoot.find(".badge-status").first().text().trim() || $(".badge-status").first().text().trim();
    ...
    parseChapters(data) {
      ...
      for (const chapter of chaptersData) {
        ...
        for (const translation of translations) {
          ...
          const score = (translation?.views ?? 0) * 1e3 + (translation?.likes ?? 0);
      ...
      for (const entry of chapterMap.values()) {
        ...
        const volume = volumeRaw ? parseFloat(volumeRaw) : void 0;
        ...
        chapters.push(App.createChapter({
          ...
          name,
          chapNum,
          volume: Number.isNaN(volume ?? NaN) ? void 0 : volume,
          time: entry.time,
          langCode: "en"
        }));
      }
    ...
    extractTitle($) {
      ...
      return ogTitle.replace(/\s+-\s+Manga Ball$/i, "").replace(/\s+Online Free\s+-\s+.*/i, "").trim() || "Unknown";
    }
    extractAltTitles($) {
    ...
    extractTags($, root) {
      ...
      scope.find("[data-tag-id]").each((_, item) => {
    ...
    extractAuthors($, root) {
      const authors = [];
      const scope = root && root.length ? root : $.root();
      scope.find("[data-person-id]").each((_, item) => {
  ...
  // src/MangaBall/MangaBall.ts
  var BASE_URL = "https://mangaball.net";
  ...
  var MangaBallInfo = {
    ...
    intents: import_types.SourceIntents.MANGA_CHAPTERS | import_types.SourceIntents.CLOUDFLARE_BYPASS_REQUIRED
  };
  var MangaBall = class {
    constructor(cheerio) {
      this.cheerio = cheerio;
      ...
      this.apiUrl = API_URL;
      this.parser = new MangaBallParser();
      this.csrfToken = "";
      this.csrfCookie = "";
      this.csrfUpdatedAt = 0;
      this.CSRF_TTL_MS = 10 * 60 * 1e3;
    ...
    async getCsrf() {
      ...
      if (this.csrfToken && fresh) {
        return { token: this.csrfToken, cookie: this.csrfCookie || void 0 };
      }
    ...
    extractCookie(headers) {
      ...
      const parts = entries.map((value) => String(value).split(";")[0].trim()).filter((value) => value.length > 0);
      return parts.length > 0 ? parts.join("; ") : void 0;
    }
    buildApiHeaders(token, cookie) {
      const headers = {
    ...
    buildFormBody(data) {
      return Object.keys(data).filter((key) => data[key] !== void 0).map((key) => {
  ...
  return __toCommonJS(MangaBall_exports);
})();
